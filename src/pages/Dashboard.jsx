...
export default function Dashboard({ clinic, session, onNavigate }) {
  ...
  const saveQuickSale = async () => {
    ...
    const todayStr = localToday()
    try {
      const clientNote = quickClient.name
        ? ' — ' + quickClient.name + (quickClient.phone ? ' ' + quickClient.phone : '')
        : ''
      ...
      setTimeout(() => { resetQuickSale(); fetchAll() }, 2000)
    } catch (e) { console.error(e) }
  ...
  // Plan: plus tiene acceso a todo lo de pro
  const planAllows = (required) => {
    if (!required) return true
    if (required === 'pro')  return plan === 'pro'  || plan === 'plus'
    if (required === 'plus') return plan === 'plus'
    return false
  }
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              ...
              { icon: 'ti-door-enter',    label: 'Mostrador',      color: '#FEF3C7', iconColor: '#D97706', action: 'walkin' },
            ...
            ].filter(a => planAllows(a.plan)).map(a => (
              <button key={a.label}
                onClick={() => {
                  if (a.action === 'quick_sale') return setShowQuickSale(true)
                  if (a.action === 'walkin')     return onNavigate('patients', 'walkin')
                  onNavigate(a.action, a.label === 'Nuevo paciente' ? 'new' : null)
                }}
      ...
      {panel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', justifyContent: 'flex-end' }}
          ...
          <div style={{ width: 420, background: 'white', height: '100%', overflowY: 'auto', padding: 24, boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
            ...
            {panelLoading
              ? <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 40 }}>Cargando...</p>
              : panelData.length === 0
                ? (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--purple-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              {appt.pets?.photo_url
                                ? <img src={appt.pets.photo_url} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 8 }} />
        ...
        <div onClick={e => e.target === e.currentTarget && setShowQuickSale(false)}
          ...
          <div style={{ background: 'white', borderRadius: 20, width: '100%', maxWidth: 480, maxHeight: '85vh', overflow: 'auto', padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={resetQuickSale} style={{ background: 'var(--purple-light)', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {quickSuccess ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={resetQuickSale} className="btn btn-secondary" style={{ flex: 1, justifyContent: 'center' }}>Cancelar</button>
