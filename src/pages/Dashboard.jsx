...
export default function Dashboard({ clinic, session, onNavigate }) {
  ...
  const isFuture = (dateStr) => dateStr > new Date().toISOString().slice(0,10)
            ...
            {[
              ...
              <button key={a.label} onClick={() => onNavigate(a.action, a.label === 'Nuevo paciente' ? 'new' : null)}
                style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, padding:'16px 12px',
                  borderRadius:12, border:'1px solid var(--border)', background:a.color, cursor:'pointer', transition:'transform 0.15s' }}
          ...
          <div style={{ width:420, background:'white', height:'100%', overflowY:'auto', padding:24, boxShadow:'-4px 0 24px rgba(0,0,0,0.12)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
              <p style={{ fontSize:17, fontWeight:800, margin:0 }}>
                {panel === 'pending' ? '⏳ Citas pendientes' : '📅 Todas las citas'}
              </p>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {panelData.map(appt => (
                  <div key={appt.id} style={{
                    ...
                    background: isToday(appt.date) ? '#EDE9FE' : isFuture(appt.date) ? 'white' : '#F8F8F8',
                    borderLeft: isToday(appt.date) ? '3px solid #6B21A8' : '3px solid var(--border)'
                  }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <p style={{ fontSize:14, fontWeight:700, margin:0 }}>{appt.pets?.name || 'Mascota'}</p>
                      </div>
                      <span className={`badge ${statusClass(appt.status)}`}>{statusLabel(appt.status)}</span>
                    </div>
                    ...
                    {appt.price && (
                      <p style={{ fontSize:12, fontWeight:700, color:'var(--purple)', margin:'6px 0 0' }}>${appt.price}</p>
                    )}
                  </div>
                ))}
              </div>
